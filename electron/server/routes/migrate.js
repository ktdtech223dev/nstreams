...
router.post('/migrate/import', (req, res) => {
  try {
    ...
    // Build a username→id map from THIS database
    const userMap = {};
    ...
    // Track tmdb_id+type → local content_id mapping
    const contentIdMap = {};
    // Pre-populate with existing content
    for (const c of db.prepare('SELECT id, tmdb_id, type FROM content WHERE tmdb_id IS NOT NULL').all()) {
      contentIdMap[`${c.tmdb_id}:${c.type}`] = c.id;
    }

    const insertContentTx = db.transaction(() => {
      for (const c of content) {
        const key = `${c.tmdb_id}:${c.type}`;
        if (!contentIdMap[key] && c.tmdb_id) {
          const info = upsertContent.run(c);
          if (info.changes) {
            const row = db.prepare('SELECT id FROM content WHERE tmdb_id=? AND type=?').get(c.tmdb_id, c.type);
            if (row) { contentIdMap[key] = row.id; importedContent++; }
          } else {
            const row = db.prepare('SELECT id FROM content WHERE tmdb_id=? AND type=?').get(c.tmdb_id, c.type);
            if (row) contentIdMap[key] = row.id;
          }
        }
      }
    });
    ...
    const insertWlTx = db.transaction(() => {
      for (const w of watchlist) {
        ...
        if (!userId) continue; // unknown user on this DB — skip

        // Find content by tmdb_id if available, otherwise skip
        let contentId = null;
        // Look it up from local content table using the exported row data
        const srcContent = content.find(c => c.id === w.content_id);
        if (srcContent?.tmdb_id) {
          contentId = contentIdMap[`${srcContent.tmdb_id}:${srcContent.type}`];
        }
    ...
    const insertProgressTx = db.transaction(() => {
      for (const ep of episodeProgress) {
        ...
        if (!userId) continue;
        const srcContent = content.find(c => c.id === ep.content_id);
        if (!srcContent?.tmdb_id) continue;
        const contentId = contentIdMap[`${srcContent.tmdb_id}:${srcContent.type}`];
        if (!contentId) continue;
    ...
    const insertActivityTx = db.transaction(() => {
      for (const af of activityFeed) {
        ...
        if (!userId) continue;
        const srcContent = content.find(c => c.id === af.content_id);
        let contentId = null;
        if (srcContent?.tmdb_id) {
          contentId = contentIdMap[`${srcContent.tmdb_id}:${srcContent.type}`];
        }
        ...
        const info = insertActivity.run({
          user_id: userId, content_id: contentId || null,
    ...
    res.json({
      ok: true,
      imported: { content: importedContent, watchlist: importedWatchlist, episodeProgress: importedProgress, activity: importedActivity },
    });
  } catch (e) {
